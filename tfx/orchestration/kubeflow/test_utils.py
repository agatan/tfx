...
class BaseKubeflowTest(tf.test.TestCase):
  ...
  @classmethod
  def _setup_mlmd_port_forward(cls):
  ...
  def _get_artifact_with_type(self, type_name: Text):
  ...
  def _get_execution_by_pipeline_name(self, pipeline_name: Text):
  ...
  def _get_execution_by_pipeline_name_and_state(self, pipeline_name: Text,
                                                state: Text):
    ...
    executions = self._get_execution_by_pipeline_name(pipeline_name)
    result = []
    for e in executions:
